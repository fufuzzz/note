...
-- insert into star(name,money,province,age,sex) values('十一',100,"深圳",100,0);
-- insert into star(name,money,province,age,sex) values('十二',100,"深圳",100,0);
-- insert into star(name,money,province,age,sex) values('十三',100,"北京",100,0);
-- select province,count(*) from star group by province order by count(*) desc limit 1,1;

-- 7. 第6题考虑如有多个第1和第2多的省份。 
-- insert into star(name,money,province,age,sex) values('十四',100,"北京",100,0);
-- insert into star(name,money,province,age,sex) values('十五',100,"江西",100,0);
-- insert into star(name,money,province,age,sex) values('十六',100,"江西",100,0);
-- insert into star(name,money,province,age,sex) values('十七',100,"江西",100,0);

-- select province, count(*) from star group by province having count(*)= 
-- (select distinct count(*) from star group by province order by count(*) desc limit 1,1);
...
join category c 
on g.categoryid=c.id and c.name='水果'; 

-- 5, 找出泡泡堂商品所属的分类名称
select c.name from goods g
join category c 
on g.categoryid=c.id and g.name="泡泡堂";
 
-- 6, 找出每个分类的分类名称及分类的商品数量 
select c.name,count(g.id) from goods g
...
-- 9, 找出比'娃哈哈'价格低的商品 
select * from goods 
where price<(select price from goods where name="娃哈哈");
